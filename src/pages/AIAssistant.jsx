                ...
                <button key={i} className="chip"
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      ...
                      const user = JSON.parse(localStorage.getItem("currentUser") || "{}");
                      const userEmail = user.email || "default";
                      fetch(`${API}/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:q, user_id: userEmail})})
                        .then(r=>r.json())
                        .then(d=>setMessages(prev=>[...prev,{role:"bot",text:d.answer}]))
                        .catch(()=>setMessages(prev=>[...prev,{role:"bot",text:"❌ Connection error"}]))
